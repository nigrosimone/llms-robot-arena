 ...
 <section id="panel-arena" class="panel">
  ...
  <div class="arena-layout">
   <div class="match-surface">
    ...
    <div class="terrain-legend" aria-label="Arena cells"><span><i class="legend-recharge"></i>Recharge +60</span><span><i class="legend-hole"></i>Hole: instant loss</span><span><i class="legend-flame"></i>Grate: warning then flame</span><span><i class="legend-collapse"></i>Red flash: floor collapse</span></div>
...
function renderFrame(frame) {
  ...
  const collapsing = (frame.cells ?? []).filter(c => c.type === "collapse" && c.state === "warning");
  const alert = $("#collapse-warning");
  alert.hidden = collapsing.length === 0;
  if (collapsing.length) {
    const message = `FLOOR COLLAPSE IN ${Math.ceil(Math.min(...collapsing.map(c => c.timeUntilChange)))} S`;
          ...
          .map((e) => {
            const label =
              e.type === "collapse-warning"
                ? `Floor cell ${e.cell} unstable: 3-second warning`
                : e.type === "collapse"
                  ? `Floor cell ${e.cell} collapsed`
                : e.type === "impact"
