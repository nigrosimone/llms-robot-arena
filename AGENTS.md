...
The live arena is [llms-robot-arena.sndesign.it](https://llms-robot-arena.sndesign.it/), a custom domain of the GitHub Pages site (a CNAME to `nigrosimone.github.io`; the old `nigrosimone.github.io/llms-robot-arena/` links are redirected by GitHub). The `conformity-and-engine` workflow runs installation, tests and a production build on **Ubuntu** for pushes and pull requests. After successful checks on `main`, it uploads only `dist/` and deploys it to the `github-pages` environment. Pull requests and other branches do not deploy. To redeploy manually, run this workflow from the repository's **Actions** tab with `main` selected.
...
When the session ends the generator measures the controller itself, whatever the agent reported: the conformity gate with the fuel budget and the standard 20-match series against Baseline. A controller admitted by the gate is registered as is: the source in `packages/bots/<id>.js`, the entry in `bots.json` with provenance `iterative`, the local results in `results/<id>/`, the JSONL transcript in `artifacts/generations/` (not versioned) and a versioned manifest in `generations/<id>.json` with harness version, model, thinking, prompt and AGENTS.md digests, engine version, turns, tokens, the cost estimate, the source digest, the gate verdict and the measured series. A controller the gate refuses is kept in `generations/failed/` with its manifest and stays out of the catalog; a session that ends without a controller registers nothing. Rerunning a model is a new id, never a replacement.
